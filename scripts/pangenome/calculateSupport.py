...
def main(alignment, pangenome, output):
    ...
    with open(alignment, "r") as alignmentFile, open(
        ...
        for line in alignmentFile.read().splitlines()[1:]:
            ...
            pathStart = (
                int(data[7]) if data[5][0] == ">" else pathLength - int(data[8])
            )  # +1#SEE GAF SPECIFICATION
            ...
            for cigarInstructionIndex, cigarInstruction in enumerate(cigarSplit):
                ...
                for cigarCounter in range(
                    cigarLength
                ):  # do x times where x is the cigar instruction count
                    ...
                    if positionOnNode > currentNodeLength - 1:
                        ...
                        if (cigarCounter == cigarLength - 1) and (
                            cigarInstructionIndex == len(cigarSplit) - 1
                        ):
            ...
            if cigarInstructionIndex != len(cigarSplit) - 1:
                ...
                logFile.write(
                    "Did not handle all cigars: {}/{}".format(
                        cigarInstructionIndex + 1, len(cigarSplit)
                    )
                )
            elif cigarLength < cigarCounter:
                logFile.write("Warning in Read: {}".format(data[0]))
                logFile.write(
                    "Did not handle the final cigar comepletely: {}/{}".format(
                        cigarCounter, cigarLength
                    )
                )
            elif positionOnPath != pathEnd:
                logFile.write("Warning in Read: {}".format(data[0]))
                logFile.write(
                    "Semi-Critical Error: Did not finish the path: {}/{}, currentNode: {}".format(
                        positionOnPath, pathEnd, currentNode
                    )
                )
            elif len(path) != 0:
                logFile.write("Warning in Read: {}".format(data[0]))
                logFile.write(
                    "Critical Error: Path is not empty!!! : {}".format(
                        [
                            "{} (length:{})".format(n, len(node2seq[currentNode]))
                            for n in path
                        ]
                    )
                )
