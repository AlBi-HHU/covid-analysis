...
def main():
    parser = argparse.ArgumentParser(
        description="Convert a bcalm-generated FASTA to a GFA.",
    ...
    with open(args.inputFilename) as f:
        ...
        for line in f:
            ...
            if line[0] == ">":
                if (
                    firstLine != 0
                ):  # if it's not the firstline in the input file, we store the input in GFA format in the output file
                ...
                for i in range(1, len(a)):
                    ...
                    if (
                        a[i][0:2] == "MA"
                    ):  # previous bcalm2 versions had "MA=[xxx]" optional tag as well, kept it just for compatibility, and reformated
                        ...
                        if args.single_directed:
                            ...
                            elif name == b[2] and not (
                                b[1] == b[3] == "-"
                            ):  # manage links between the same unitig
                                links.append(
                                    "L\t"
                                    + name
                                    + "\t"
