            ...
            while True:
                ...
                if len(data) == 1:  # This is another header for the next sample
                    ...
                    ret[sample][data[0]] = (
                        data[3],
                        data[2],
                    )  # key: position, value: nanopore variant call
                    currentPosition += 1
    return ret


with open(snakemake.input["method1"], "r") as meth1file, open(
    snakemake.input["method2"], "r"
) as meth2file, open(snakemake.output[0], "w") as outfile:
    ...
    for sample in set(meth1data.keys()).union(meth2data.keys()):
        ...
        if sample not in meth1data:
            ...
            outfile.write(
                "{}\t{}\t{}\t{}\n".format(
                    "Position",
                    snakemake.params["method1name"],
                    snakemake.params["method2name"],
                    "iVar",
                )
            )
                ...
                if k not in meth1data[sample]:
                    outfile.write(
                        "{}\t{}\t{}\t{}\n".format(
                            k,
                            "Not called",
                            meth2data[sample][k][0],
                            meth2data[sample][k][1],
                        )
                    )
                    ...
                    outfile.write(
                        "{}\t{}\t{}\t{}\n".format(
                            k,
                            meth1data[sample][k][0],
                            "Not called",
                            meth1data[sample][k][1],
                        )
                    )
