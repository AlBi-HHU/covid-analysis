...
for pancovF, ivarF,nanoporeF,pileupF in zip(snakemake.input["pancov"], snakemake.input["ivar"],snakemake.input['nanopore'],snakemake.input['illuminaPileups']):
    ...
    for record in reader:
        ...
        localtuples.append((pancovF, record.POS, "pancov", alleleFrequency,record.ALT[0].value))
    ...
    for record in reader:
        ...
        localtuples.append((pancovF, record.POS, "nanopolish", alleleFrequency,record.ALT[0].value))
        pileupPositions[record.POS] = record.ALT[0].value
...
for f in df["file"].unique():
    ...
    for m in ['pancov','ivar','nanopolish']:
        ...
        bar = base.mark_bar().encode(
            x=alt.X('diff:Q', bin=True, axis=None, scale=alt.Scale(domain=[0, 1])),
            y='count()'
        )
